...
def set_configuration():
    ...
    cfg.MODEL.WEIGHTS          = os.path.join("model/model_bbox_e10k_class10_19Aug-faster_rcnn_R_50_FPN_3x.pth")
...
def load_images_ann_dicts(basepath, splits_per_dataset):
    ...
    for idx, regions_data in enumerate(splits_per_dataset):
        ...
        # img_path = os.path.join(basepath,"train_short/Czech/images")
        ...
        for img_id, img_filename in enumerate(img_file_list):
            ...
            if os.path.isfile(os.path.join(ann_path, ann_file)):
                ...
                for obj in root.iter('object'):
                    ...
                    new_row = {'frame': img_filename, 'xmin': int(xmin), 'ymin': int(ymin),
                            'xmax': int(xmax), 'ymax': int(ymax), 'label': cls_name,
                            'full_file': os.path.join(img_path, img_filename)}
