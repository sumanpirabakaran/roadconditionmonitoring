...
def main():
    ...
    app_mode = st.sidebar.selectbox("Choose the app mode",
        ["Show instructions", "Run the app", "Show the source code", "Check your own file"])
    if app_mode == "Show instructions":
        ...
        readme_text.empty()
        run_the_app()
    elif app_mode == "Check your own file":
        readme_text.empty()
        detect_image()
...
def run_the_app():
    ...
    # An amazing property of st.cache_datad functions is that you can pipe them into
...
def detect_image():
    st.write('You need to upload your clicked image of a damaged road')
    uploaded_file = st.file_uploader("Pick a file", type=("png", "jpg"))
    ...
    if uploaded_file:
        ...
        boxes = model_bbox.predict_rdd(image, confidence_threshold)
        draw_image_with_boxes(image, boxes, "Real-time Road Damage Detection",
            "**Faster RCNN Resnet 50 Model** (overlap `%3.1f`) (confidence `%3.1f`) -`%s`" % (overlap_threshold, confidence_threshold, ""))
